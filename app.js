...
function getRandomSafeSpot() {
  return randomFromArray([
    { x: 1, y: 4 }, { x: 2, y: 4 }, { x: 1, y: 5 }, { x: 2, y: 6 }, { x: 2, y: 8 }, { x: 2, y: 9 }, { x: 4, y: 8 }, { x: 5, y: 5 },
    { x: 5, y: 8 }, { x: 5, y: 10 }, { x: 5, y: 11 }, { x: 11, y: 7 }, { x: 12, y: 7 }, { x: 13, y: 7 }, { x: 13, y: 6 }, { x: 13, y: 8 },
    { x: 7, y: 6 }, { x: 7, y: 7 }, { x: 7, y: 8 }, { x: 8, y: 8 }, { x: 10, y: 8 }, { x: 8, y: 8 }, { x: 11, y: 4 },
  ]);
}
...
(function () {
  ...
  function handleArrowPress(xChange = 0, yChange = 0) {
    ...
    if (!isSolid(newX, newY)) {
      ...
      walkSound.play();  // Play the walk sound
    }
  }
  ...
  firebase.auth().onAuthStateChanged((user) => {
    if (user) {
      ...
      const { x, y } = getRandomSafeSpot();
      playerRef.set({
        id: playerId, name, direction: "right", color: randomFromArray(playerColors), x, y, coins: 0,
      });
